...
export const PlanPickerKit = ({
  ...
  // Update selling plan when variant is changed
  useEffect(() => {
    const newSellingPlan = sellingPlanAllocations.find(
      (sp) => sp.sellingPlan.name === sellingPlan?.name,
    )?.sellingPlan;
  ...
  const navigateToSinglePack = () => {
    const clonedSearchParams = new URLSearchParams(currentSearchParams);
    clonedSearchParams.set(KIT_REFILL_QUANTITY_OPTION_NAME, 'Single');
    navigate(`${pathname}?${clonedSearchParams.toString()}`);
  };
    ...
    >
      ...
      >
        ...
        <div className="mr-auto uppercase">
          One-Time <span className="hidden md:inline">Purchase</span>
        </div>
        <span className="text-sm px-2 py-1 peer-checked:bg-beige bg-background rounded-lg peer-checked:text-background text-white font-light">
          Save $59
        </span>
        <Money
          ...
          as="span"
          className="opacity-50 strike md:text-xl sm:text-lg text-base"
        />
        <Money
          withoutTrailingZeros
      ...
      >
        ...
        />
        <span className="block w-4 h-4 rounded-full peer-checked:bg-beige bg-background"></span>
        <span className="mr-auto uppercase">With Refill</span>
        <span className="text-sm px-2 py-1 peer-checked:bg-beige bg-background rounded-lg peer-checked:text-background text-white font-light">
          Save $100
        </span>
        <Money
        ...
        {sellingPlan && (
          <div className="w-full px-6 py-1">
            <div className="flex flex-col gap-4 border-t border-t-beige py-3 text-base">
              <OptionSelector
                optionName={KIT_REFILL_QUANTITY_OPTION_NAME}
                product={product}
...
const OptionSelector = ({optionName, product, selectedVariant}) => {
  const {pathname} = useLocation();
  const [currentSearchParams] = useSearchParams();

  const searchParamsWithDefaults = useMemo(() => {
    const clonedParams = new URLSearchParams(currentSearchParams);

    for (const {name, value} of product.variants.nodes[0].selectedOptions) {
    ...
    return clonedParams;
  }, [currentSearchParams, product]);

  const optionGroup = product.options.find(
    (option) => option.name === optionName,
  );

  const activeOption = selectedVariant.selectedOptions.find(
    (option) => option.name === optionName,
  ).value;
    ...
    <div className="flex flex-col md:flex-row md:items-center gap-2">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {optionGroup.values.map((value) => {
          const id = `option-${optionGroup.name}-${value}`;
          const clonedSearchParams = new URLSearchParams(
            searchParamsWithDefaults,
          );
          const checked = clonedSearchParams.get(optionName)
            ? clonedSearchParams.get(optionName) === value
            : activeOption === value;
          clonedSearchParams.set(optionGroup.name, value);
          ...
          return (
            <Link
              ...
              prefetch="intent"
              replace
              to={`${pathname}?${clonedSearchParams.toString()}`}
              className={clsx('font-light', {
                'bg-secondary text-white border border-secondary rounded-full px-2 md:px-4 md:py-1':
                  checked,
                'bg-beige text-black border border-accent rounded-full px-2 md:px-4 md:py-1':
                  !checked,
              })}
            >
              {value}
